...
public class CaughtBallChangerItem extends PokemonUseItem{
    ...
    public ActionResult processInteraction(ItemStack itemStack, PlayerEntity player, PokemonEntity target, Pokemon pokemon) {
        ...
        if (currentball == selectedBall) {
            player.sendMessage(Text.literal("").append(pokemon.getDisplayName(false)).append(" has the same caught ball"));
            return ActionResult.FAIL;
        }
        // change caught ball
        pokemon.setCaughtBall(selectedBall);
        player.sendMessage(Text.literal("").append(pokemon.getDisplayName(false)).append("'s caught ball has been changed to " + selectedBall.item().getName().getString()));
